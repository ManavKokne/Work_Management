...
let hasEnsuredTaskReporterEmailColumn = false;
let hasEnsuredReportPhotoColumn = false;
let hasEnsuredReportReporterEmailColumn = false;
...
export async function ensureTaskReporterEmailColumn() {
  ...
  if (hasEnsuredTaskReporterEmailColumn) {
    return;
  }
  ...
  if (count === 0) {
    await query(`ALTER TABLE tasks ADD COLUMN reporter_email VARCHAR(255) NULL AFTER task_reported_by`);
  }

  hasEnsuredTaskReporterEmailColumn = true;
}

export async function ensureReportReporterEmailColumn() {
  await ensureCoreTables();

  if (hasEnsuredReportReporterEmailColumn) {
    return;
  }
  ...
  if (count === 0) {
    await query(`ALTER TABLE reports ADD COLUMN reporter_email VARCHAR(255) NULL AFTER location`);
  }

  hasEnsuredReportReporterEmailColumn = true;
}
